...
var (
	output = pflag.StringP("output", "o", "-", "Output for swagger functions; '-' means stdout (default)")
	source = pflag.StringP("source", "s", "", "The source file to parse type from")
	verify       = pflag.BoolP("verify", "v", false, "Verifies if the given source file has documentation for all types and fields")
)
...
func main() {
	...
	if *source == "" {
		fmt.Println("Please specify the source file via the -s flag")
		os.Exit(-1)
	}
	...
	if *output == "-" {
		...
		if err != nil {
			fmt.Printf("Couldn't open %v: %v", *output, err)
		}
		defer file.Close()
		funcOut = file
	}

	docsForTypes := ParseDocumentationFrom(*source)

	if *verify {
		rc, err := VerifySwaggerDocsExist(docsForTypes, funcOut)
		if err != nil {
	...
	if len(docsForTypes) > 0 {
		if err := WriteSwaggerDocFunc(docsForTypes, funcOut); err != nil {
